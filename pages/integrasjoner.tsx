...
const McpKeyManager = () => {
  ...
  const createKey = async () => {
    ...
    try {
      ...
      await mutateKeys((current = []) => [created, ...current], {
        revalidate: false,
      });
    ...
    <div className={styles.keyManager}>
      ...
      {(error || keysError) && (
        <p className={styles.copyError} role="alert">
          {error ?? "Kunne ikke hente API-nøkler."}
        </p>
      )}
      {keysLoading && <p className={styles.keyStatus}>Henter API-nøkler…</p>}
      {!keysLoading && !keysError && keys?.length === 0 && (
        <p className={styles.keyStatus}>Du har ingen API-nøkler ennå.</p>
      )}
      {keys && keys.length > 0 && (
